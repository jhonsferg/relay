...
//
// oauth_test.go already exercises the full RFC 6749 client-credentials wire
// format (form-encoded request, JSON response, error parsing, refresh) via
// testutil.MockServer. This test adds an end-to-end check against a real,
// widely-deployed identity provider: proves the token request this package
// builds is accepted by real Keycloak, and that a real (not synthetic)
// token response - which includes extra fields a hand-written mock might
// not (not-before-policy, refresh_expires_in, scope, session_state, ...) -
// round-trips correctly through this package's JSON parsing.
package oauth_test
...
// grant enabled, imported automatically at container startup.
const kcRealmExport = `{
  "realm": "` + kcRealm + `",
  "enabled": true,
  "clients": [
...
func TestDocker_WithClientCredentials_RealKeycloak(t *testing.T) {
...
func TestDocker_WithClientCredentials_WrongSecretRejected(t *testing.T) {
	base := startKeycloak(t)
	tokenURL := base + "/realms/" + kcRealm + "/protocol/openid-connect/token"
